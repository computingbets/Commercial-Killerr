...
/***/ function(module, exports, __webpack_require__) {
	...
	var onGetChannelAds = function onGetChannelAds(event) {
	  ...
	  api.getChannelAds(channelTargeted).then(api.updateChannelAds).then(ui.updateChannelAdsSuccess).catch(function (error) {
	...
	var onGetChannelProg = function onGetChannelProg(event) {
	  ...
	  event.preventDefault();
	  var channelTargeted = $('.commercial-button').text();
	  api.getChannelProg(channelTargeted).then(api.updateChannelProg).then(ui.updateChannelProgSuccess).catch(function (error) {
	    return console.error(error);
	  });
	};
